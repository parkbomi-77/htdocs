...
add_filter( 'woocommerce_email_classes', array( 'MSM_Email_Authenticate', 'woocommerce_email_classes' ) );
add_action( 'user_register', array( 'MSM_Email_Authenticate', 'user_register' ) );
...
function msm_pre_get_avatar( $image, $id_or_email, $args ) {
...
add_filter( 'pre_get_avatar', 'msm_pre_get_avatar', 10, 3 );

function msm_maybe_change_avatar( $avatar, $id_or_email, $size, $default, $alt, $args ) {
	if ( is_numeric( $id_or_email ) ) {
