...
class MSM_Admin_Post_types {
	...
	public static function mshop_role_request_manage_posts_custom_column( $column, $post_id ) {
		switch ( $column ) {
			...
			case 'msm_action':
				$status      = get_post_status( $post_id );
				$approve_url = wp_nonce_url( admin_url( 'admin-ajax.php?action=' . MSM()->slug() . '-approved&request_id=' . $post_id ), 'msm-role-application' );
				$reject_url  = wp_nonce_url( admin_url( 'admin-ajax.php?action=' . MSM()->slug() . '-rejected&request_id=' . $post_id ), 'msm-role-application' );
				if ( 'mshop-apply' === $status ) {
