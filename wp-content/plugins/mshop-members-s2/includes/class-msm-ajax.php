...
class MSM_Ajax {
	...
	public static function submit() {
		...
		if ( isset( $_REQUEST['form_ids'] ) ) {
			...
			try {
				...
				if ( is_user_logged_in() && ! wp_verify_nonce( $_REQUEST['msm_nonce'], 'mshop-members-s2-' . end( $form_ids ) ) ) {
	...
	public static function update_msm_settings_social() {
		if ( ! current_user_can( 'manage_options' ) ) {
			die();
		}
	...
	public static function update_msm_role_settings() {
		if ( ! current_user_can( 'manage_options' ) ) {
			die();
		}
