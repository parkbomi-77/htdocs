...
if ( ! class_exists( 'MSSMS_SMS' ) ) {
	...
	class MSSMS_SMS {
		...
		public static function send_sms( $type, $title, $message, $recipients, $send_no = '', $request_date = '', $is_auth = false ) {
			if ( empty( $send_no ) ) {
			...
			return MSSMS_API_SMS::send_message( $type, $send_no, $title, $message, $recipients, $request_date, $is_auth );
		}
		...
		public static function send_vact_info( $order_id, $rcv_num, $vacc_bank_name, $vacc_num, $vacc_name, $vacc_input_name, $vacc_date ) {
			try {
				$order   = wc_get_order( $order_id );
				$message = self::get_message_by_order_status( 'pafw-send-vact-info', 'user', $order );
